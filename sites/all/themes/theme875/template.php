...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme875
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme875_clear_registry')) {
	// Rebuild .info data.
 ...
 */
function theme875_html_head_alter(&$head_elements) {
 ...
 */
function theme875_preprocess_search_block_form(&$vars) {
 ...
 */
function theme875_preprocess(&$vars, $hook) {
	$vars['theme875_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme875_preprocess_html(&$vars) {
	$vars['doctype'] = _theme875_doctype();
	$vars['rdf'] = _theme875_rdf($vars);
	...
	if (!$vars['is_front']) {
		...
		if (isset($page_name)) {
			$vars['classes_array'][] = theme875_id_safe('page-' . $page_name);
		}

		$vars['classes_array'][] = theme875_id_safe('section-' . $section);
 ...
 */
function theme875_preprocess_page(&$vars) {
 ...
 */
function theme875_preprocess_maintenance_page(&$vars) {
	...
	drupal_add_css(drupal_get_path('theme', 'theme875') . '/css/maintenance-page.css');
}
 ...
 */
function theme875_preprocess_node(&$vars) {
	// Striping class
 ...
 */
function theme875_preprocess_block(&$vars, $hook) {
 ...
 */
function theme875_menu_tree($vars) {
 ...
 */
function theme875_field__taxonomy_term_reference($vars) {
	$output = '';
 ...
 */
function theme875_breadcrumb($vars) {
	
	$breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
	$show_breadcrumb = theme_get_setting('theme875_breadcrumb_display');
	if ($show_breadcrumb == 'yes') {
	
		if (theme_get_setting('theme875_breadcrumb_hideonlyfront')) {
			$condition = count($breadcrumb) > 1;
		} else {
			$condition = !empty($breadcrumb);
		}

		if(theme_get_setting('theme875_breadcrumb_showtitle')) {
		...
		$separator = theme_get_setting('theme875_breadcrumb_separator');
 ...
 * @return bool
 */
function theme875_tabs_float() {
	$float = (bool) theme_get_setting('theme875_tabs_float');
	$float_node = (bool) theme_get_setting('theme875_tabs_node');
...
function theme875_id_safe($string) {
	// Strip accents
 ...
 * @ingroup themeable
 */
function theme875_menu_link(array $vars) {
	...
	$element['#attributes']['class'][] = theme875_id_safe($element['#title']);
 ...
 */
function theme875_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme875_menu_local_tasks(&$vars) {
	$output = '';
